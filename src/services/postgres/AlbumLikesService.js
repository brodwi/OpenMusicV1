...
class AlbumLikesService {
  ...
  async getAlbumLikes(albumId) {
    try {
      ...
      const query = {
        text: 'SELECT * FROM user_album_likes WHERE albums_id = $1',
        values: [albumId],
      };
  ...
  async deleteLike(albumId, userId) {
    const query = {
      text: 'DELETE FROM user_album_likes WHERE albums_id = $1 AND users_id = $2',
  ...
  async verifyUserLiked(albumId, userId) {
    await this._albumService.getAlbumsById(albumId);
    const query = {
      text: 'SELECT * FROM user_album_likes WHERE albums_id = $1 AND users_id = $2',
      values: [albumId, userId],
    };
    const result = await this._pool.query(query);
